...
float iou_calc(const HailoBBox & box_1, const HailoBBox & box_2)
{
  ...
  const float width_of_overlap_area = std::min(box_1.xmax(), box_2.xmax()) - std::max(
    box_1.xmin(), box_2.xmin());
  const float height_of_overlap_area = std::min(box_1.ymax(), box_2.ymax()) - std::max(
...
/**
     ...
     */
void nms(
  std::vector<HailoDetection> & objects, const float iou_thr,
    ...
    if (objects[index].get_confidence() != 0.0f) {
      for (uint jindex = index + 1; jindex < objects.size(); jindex++) {
        if ((should_nms_cross_classes ||
          (objects[index].get_class_id() == objects[jindex].get_class_id())) &&
