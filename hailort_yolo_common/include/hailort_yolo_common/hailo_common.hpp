...
inline void add_classification(
  HailoROIPtr roi, std::string type, std::string label,
  float confidence, int class_id = NULL_CLASS_ID)
{
  add_object(
    roi,
...
inline HailoDetectionPtr add_detection(
  HailoROIPtr roi, HailoBBox bbox, std::string label,
  float confidence, int class_id = NULL_CLASS_ID)
{
...
inline std::vector<HailoClassificationPtr> get_hailo_classifications(
  HailoROIPtr roi,
  std::string classification_type = "")
{
  ...
  for (auto obj : objects) {
    ...
    if (classification_type.empty() ||
      classification_type.compare(classification->get_classification_type()) == 0)
    {
...
inline std::vector<HailoUniqueIDPtr> get_hailo_unique_id_by_mode(
  HailoROIPtr roi,
  hailo_unique_id_mode_t mode)
{
  std::vector<HailoObjectPtr> objects = roi->get_objects_typed(HAILO_UNIQUE_ID);
  std::vector<HailoUniqueIDPtr> unique_ids;
...
inline void flatten_hailo_roi(HailoROIPtr roi, HailoROIPtr parent_roi, hailo_object_t filter_type)
{
  ...
  for (uint index = 0; index < objects.size(); index++) {
    if (objects[index]->get_type() == filter_type) {
      ...
      sub_obj_roi->set_bbox(
        std::move(
          create_flattened_bbox(
            sub_obj_roi->get_bbox(),
            roi->get_bbox())));
...
/**
     ...
     */
inline HailoPoint fixate_point_with_bbox(
  HailoROIPtr hailo_roi, HailoPoint & point,
  const HailoBBox & fixate_bbox)
{
  ...
  return HailoPoint(
    (xmin - fixate_bbox.xmin()) / fixate_bbox.width(),
