...
public:
  ...
  : m_x(x), m_y(y), m_confidence(assure_normal(confidence)) {}
...
public:
  ...
  : HailoObject(other), m_sub_objects(other.m_sub_objects) {}
  ...
  void remove_object(HailoObjectPtr obj)
  {
    ...
    m_sub_objects.erase(
      std::remove(
        m_sub_objects.begin(),
        m_sub_objects.end(), obj), m_sub_objects.end());
  }
...
public:
  ...
  : m_bbox(bbox), m_scaling_bbox(HailoBBox(0.0, 0.0, 1.0, 1.0)) {}
  virtual ~HailoROI() = default;
  HailoROI(HailoROI && other) noexcept
  : HailoMainObject(other), m_bbox(std::move(other.m_bbox)),
    m_scaling_bbox(std::move(other.m_scaling_bbox)) {}
...
public:
  HailoTileROI(
    HailoBBox bbox, uint index, float overlap_x_axis, float overlap_y_axis, uint layer,
    hailo_tiling_mode_t mode)
  : HailoROI(bbox), m_index(index), m_overlap_x_axis(overlap_x_axis), m_overlap_y_axis(
      overlap_y_axis), m_layer(layer), m_mode(mode) {}
  // Move constructor
...
public:
  ...
  : HailoROI(bbox), m_confidence(assure_normal(confidence)), m_label(label), m_class_id(
      NULL_CLASS_ID) {}
  /**
   * @brief Construct a new New Hailo Detection object
   *
...
public:
  ...
  HailoClassification(
    ...
    float confidence)
  : m_confidence(assure_normal(confidence)), m_classification_type(classification_type), m_label(
      label), m_class_id(NULL_CLASS_ID) {}
...
public:
   ...
   */
  HailoLandmarks(
    std::string landmarks_name, float threshold = 0.0f,
   ...
   */
  HailoLandmarks(
    std::string landmarks_name,
    std::vector<HailoPoint> points,
    float threshold = 0.0f,
    const std::vector<std::pair<int, int>> pairs = {})
  : m_landmarks_type(landmarks_name),
  ...
  : m_landmarks_type(other.m_landmarks_type), m_points(other.m_points), m_threshold(
      other.m_threshold), m_pairs(other.m_pairs) {}
...
public:
  ...
  : m_unique_id(unique_id), m_mode(mode) {}
...
public:
  HailoDepthMask(
    std::vector<float> && data_vec, int mask_width, int mask_height,
    float transparency)
...
public:
  HailoClassMask(
    std::vector<uint8_t> && data_vec, int mask_width, int mask_height,
    float transparency)
  : HailoMask(mask_width, mask_height, transparency), m_data(std::move(data_vec)) {}
...
public:
  ...
  : HailoMask(mask_width, mask_height, transparency), m_class_id(class_id),
